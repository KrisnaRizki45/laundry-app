...
const filteredUsers = users
  .map((u, index) => {
    ...
    return {
      ...
      status: "Active",
      lastActive: "Today",
      idBackground: colorCycle[colorIndex],
    };
  })
  .filter((user) =>
        ...
        <table className="w-full border border-[#dbe1e6] rounded-xl overflow-hidden">
          <thead className="bg-white">
            <tr className="text-left text-[#111518] text-sm font-medium">
              <th className="px-4 py-3 w-[400px]">ID</th>
              ...
              {/* <th className="px-4 py-3 w-[400px]">Dibuat</th>
              <th className="px-4 py-3 w-[400px]">Diubah</th> */}
              <th className="px-4 py-3 w-60">Aksi</th>
            </tr>
          </thead>
          <tbody>
            {filteredUsers.map((user) => (
              <tr key={user.id} className="border-t border-[#dbe1e6]">
                <td className="px-2 py-3 text-sm text-[#111518]">
                  <span
                    className={`px-3 py-0.5 rounded-lg border border-gray-300 
                      ${user.idBackground} 
                      break-all inline-block max-w-[105px] text-xs text-center`}
                  >
                    {user.id}
                  </span>
                </td>
                ...
                {/* <td className="px-4 py-2 text-sm text-[#617689]">{formatDate(user.createdAt)}</td>
                <td className="px-4 py-2 text-sm text-[#617689]">{formatDate(user.updatedAt)}</td> */}
            ...
            {filteredUsers.length === 0 && (
              <tr>
                <td colSpan="6" className="text-center py-6 text-[#617689]">
                  No users found.
                </td>
              </tr>
            )}
